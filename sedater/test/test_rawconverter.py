...
# Modified:	Thu, 05.11.2015 - 19:08:48
...
class TestRawConverter(unittest.TestCase):
	...
	def test_sensor_setup(self):
		conv = rawvalidation.RawConverter([], [self.foo, self.bar])
		conv.processDatFiles()
		self.assertIsInstance(conv.initSensors[
			shared.Orientation(1).name], rawvalidation.Sensor)
		self.assertIsInstance(conv.initSensors[
			shared.Orientation(2).name], rawvalidation.Sensor)
	...
	def test_invalid_filesToConvert_parameter(self):
		conv = rawvalidation.RawConverter(
